...
class NANDA:
    ...
    def __init__(self, improvement_logic):
        ...
        self.bridge = None
        print(
            f"🤖 NANDA initialized with custom improvement logic: {improvement_logic.__name__}"
        )
    ...
    def register_custom_improver(self):
        ...
        print(
            f"🔧 Custom improvement logic '{self.improvement_logic.__name__}' registered"
        )
    ...
    def create_agent_bridge(self):
        ...
        print(
            f"✅ AgentBridge created with custom improve_message_direct: {self.improvement_logic.__name__}"
        )
    ...
    def start_server_api(
        ...
        def cleanup(signum=None, frame=None):
            ...
            if (
                hasattr(run_ui_agent_https, "bridge_process")
                and run_ui_agent_https.bridge_process
            ):
        ...
        if ssl:
            ...
            if not cert or not key:
                ...
                key = "./privkey.pem"
                print(
                    f"🔒 Using certificates from current folder: cert={cert}, key={key}"
                )
            ...
            if os.path.exists(cert) and os.path.exists(key):
                ...
                print(
                    f"💡 Make sure Let's Encrypt certificates exist for domain: {domain}"
                )
