...
def send_message():
    ...
    try:
        ...
        bridge_url = (
            f"http://localhost:{agent_port}/a2a"  # Remove /a2a since A2AClient adds it
        )
...
def list_agents():
    ...
    try:
        ...
        return (
            jsonify({"error": f"Failed to get agent list: {response.text}"}),
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=6000, help="Agent bridge port (default: 6000)"
    )
    parser.add_argument(
        "--api-port", type=int, default=5000, help="Flask API port (default: 5000)"
    )
    ...
    parser.add_argument(
        "--ssl", action="store_true", help="Enable SSL with default certificates"
    )
